...
struct DensePairMap {
    ...
    void save_(string fp) {
        ...
        for (size_t i = 0; i < n; i++) {
            // write the hash map one by one
            for (size_t node=0; node < v[i].size(); node++) {
                if (v[i][node] > 0.001){
                    //cout << i << "," << node << ":" << v[i][node] << endl;
                    o_file << i << "," << node << "," << v[i][node] << endl;
                }
            }
        }
        o_file.close();
...
namespace Eigen {
    ...
    /*
    void write_normal(const char *filename, const Matrix &matrix) {
        std::ofstream out;
        out.open(filename);
