...
from utils import MixedLinear, MixedDropout, sparse_matrix_to_torch



def calc_sim_iter(adj, decay, niter):
    
    # implemention of PARsim: Efficient partial-pairs simrank search on large networks for exact simrank computation on small-scale datasets.
    
    nnodes = adj.shape[0]
    index = (range(nnodes), range(nnodes))
    adj = adj.to_dense()
    P = F.normalize(adj, p=1, dim=0)# inverse transition matrix.
    D = torch.eye(nnodes) # correction matrix (approximate)
    S = D
...
class SimIter(nn.Module):
    
    def __init__(self, adj: sp.spmatrix, decay: float, epison: float = 1e-6, drop_prob: float = None, pre_cal_file = None, device='cpu', skip_factor = 1):
        super().__init__()
        
        # self.skip_factor = nn.Parameter(torch.FloatTensor([1.0]), requires_grad=True)
        self.skip_factor = skip_factor
        niter = 2 * round(math.log(1 / epison))
        self.device = device
    
        if pre_cal_file is None or not os.path.exists(pre_cal_file):
            self.mat = calc_sim_iter(adj, decay, niter) # calculate exact simrank matrix via iterations.
        else:
            if '.pt' in pre_cal_file:
                self.mat = torch.load(pre_cal_file)
                self.mat = self.mat.to_dense().to(torch.float64).type(torch.FloatTensor).to_sparse().to(self.device)
            elif '.npz' in pre_cal_file:
                self.mat = sp.load_npz(pre_cal_file) # sparse csr matrix
                n = self.mat.shape[0]
                self.mat = self.mat.tocoo()
                self.mat = torch.sparse.FloatTensor(torch.LongTensor([self.mat.row.tolist(), self.mat.col.tolist()]),
                                                          torch.FloatTensor(self.mat.data.astype(np.float64)), 
                                                          torch.Size([n, n])).to(self.device)
    
        # self.mat = nn.Parameter(torch.DoubleTensor(sim_mat), requires_grad = False)
        # del self.mat
    ...
    def forward(self, h):
        ...
        return torch.sparse.mm(self.dropout(self.mat), h) + self.skip_factor * h

class SimGNN(nn.Module):
    
    def __init__(self, nnodes, nfeat, hiddenunits, nclass, dropout_p, delta, propagation, propa_mode='post', simrank_guided=1):
        super(SimGNN, self).__init__()
        
        self.delta = delta
        self.propa_mode = propa_mode
        self.simrank_guided = simrank_guided
        
        if dropout_p == 0:
            self.dropout = lambda x: x
        else:
            self.dropout = MixedDropout(dropout_p)
        
        
        self.act_fn = nn.ReLU()        

        # for feat transformation
        self.fc1 = nn.Sequential(self.dropout, nn.Linear(nfeat, hiddenunits[0]), self.act_fn)
        
        for i in range(1, len(hiddenunits)-1):
            self.fc1.add_module('feat-dropout-'+str(i), self.dropout)
            self.fc1.add_module('feat_mlp-'+str(i), nn.Linear(hiddenunits[i - 1], hiddenunits[i]))
            self.fc1.add_module('feat-relu-'+str(i), self.act_fn)
        if len(hiddenunits) > 1:
            self.fc1.add_module('feat-dropout-'+str(len(hiddenunits)), self.dropout)
            self.fc1.add_module('feat_mlp-'+str(len(hiddenunits)), nn.Linear(hiddenunits[-2], hiddenunits[-1]))
                                                                            
        # parallel for adj transformation
        self.fc2 = nn.Sequential(self.dropout, nn.Linear(nnodes, hiddenunits[0]), self.act_fn)
        
        for i in range(1, len(hiddenunits)-1):
            self.fc2.add_module('feat-dropout-'+str(i), self.dropout)
            self.fc2.add_module('feat_mlp-'+str(i), nn.Linear(hiddenunits[i - 1], hiddenunits[i]))
            self.fc2.add_module('feat-relu-'+str(i), self.act_fn)
        if len(hiddenunits) > 1:
            self.fc2.add_module('feat-dropout-'+str(len(hiddenunits)), self.dropout)
            self.fc2.add_module('feat_mlp-'+str(len(hiddenunits)), nn.Linear(hiddenunits[-2], hiddenunits[-1]))
                                       
        '''
        # for feat transformation
        fcs1 = [nn.Linear(nfeat, hiddenunits[0])]
        for i in range(1, len(hiddenunits)):
            fcs1.append(nn.Linear(hiddenunits[i - 1], hiddenunits[i]))
        self.fcs1 = nn.ModuleList(fcs1)
        
        # parallel for adj transformation
        fcs2 = [nn.Linear(nnodes, hiddenunits[0])]
        for i in range(1, len(hiddenunits)):
            fcs2.append(nn.Linear(hiddenunits[i - 1], hiddenunits[i]))
        self.fcs2 = nn.ModuleList(fcs2)
        '''
        
        self.xX_p = None
        self.xA_p = None
        self.fc = nn.Linear(hiddenunits[-1], nclass)
        self.fc_ = nn.Linear(hiddenunits[-1], hiddenunits[-1])
        self.alpha_ = nn.Parameter(torch.FloatTensor([0.5]), requires_grad=True)
        #self.fc = nn.Sequential(nn.Linear(hiddenunits[-1], 32), self.act_fn, self.dropout, nn.Linear(32, nclass))
    ...
    def reset_parameters(self):
        ...
        self.fc_.reset_parameters()  

    def _transform_features(self, x, adj):
        ...
        # mix up
        x = self.dropout(self.act_fn(self.delta * xX + (1-self.delta) * xA))
        
        x = (1 - self.alpha_) * self.dropout(self.act_fn(self.fc_(x))) + self.alpha_ * x
    ...
    def forward(self, x, adj):
        ...
        if self.propa_mode == 'post':
            if self.simrank_guided:
                local_logits = self._transform_features(x, adj)
        ...
        elif self.propa_mode == 'pre':
            if self.xX_p is None or self.xA_p is None:
                self.xX_p = self.propagation(x)
                self.xA_p = self.propagation(adj)
            final_logits = self._transform_features(self.xX_p, self.xA_p)
            # final_logits = self._mlp(final_logits)       
            return final_logits
...
class H2GCN(nn.Module):
    ...
    def init_adj(self, edge_index):
        ...
        if isinstance(edge_index, SparseTensor):
            ...
            adj_t = edge_index
            adj_t = sp.csr_matrix(adj_t.to_scipy())
            adj_t[adj_t > 0] = 1
            adj_t[adj_t < 0] = 0
        ...
        adj_t = sp.csr_matrix(adj_t.to_scipy())
        adj_t2 = sp.csr_matrix(adj_t2.to_scipy())
        adj_t2 = adj_t2 - adj_t
        adj_t2[adj_t2 > 0] = 1
        adj_t2[adj_t2 < 0] = 0
