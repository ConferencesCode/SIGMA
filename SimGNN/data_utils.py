...
DATAPATH = '/media/disk2/lhy/graph/large/'
...
@torch.no_grad()
def evaluate_simgnn(model, x, adj, dataset, split_idx, eval_func, result=None, sampling=False, subgraph_loader=None):
    if result is not None:
        out = result
    else:
        model.eval()
...
def load_fixed_splits(dataset, sub_dataset):
    ...
    """
    name = dataset
    if dataset == 'fb100' and sub_dataset and sub_dataset != 'None':
        name += f'-{sub_dataset}'
