...
from models import LINK, GCN, MLP, MLPNORM_Z, SGC, GAT, SGCMem, MultiLP, MixHop, GCNJK, GATJK, H2GCN, APPNP_Net, LINK_Concat, LINKX, GPRGNN, GCNII, MLPNORM, GGCN, ACMGCN, MLPNORM_Z, WRGAT, SimIter, SimGNN
from models import *
from data_utils import normalize
import math


def parse_method(args, dataset, n, c, d, device, num_relations, adj = None):
    if args.method == 'link':
    ...
    elif args.method == 'simgnn':
        simiter = SimIter(adj=adj, decay=args.decay, epison=args.epison, drop_prob=args.dropout, pre_cal_file=args.simrank_pre_cal_file, device=device, skip_factor=args.skip_factor)
        model = SimGNN(nnodes=dataset.graph['num_nodes'], nfeat=d, hiddenunits=args.hiddenunits, nclass=c, dropout_p=args.dropout, delta=args.delta, propagation=simiter, propa_mode=args.propa_mode, simrank_guided=args.simrank_guided).to(device)
    else:
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--method', '-m', type=str, default='simgnn')
    ...
    parser.add_argument('--decay', type=float, default=0.6,
                        help='decay rate in simrank')
    parser.add_argument('--niter', type=int, default=20,
                        help='iteration amounts in exact simrank')
    parser.add_argument('--epison', type=float, default=1e-4,
                        help='epison for simrank computation')
    parser.add_argument('--hiddenunits', nargs='+', type=int, default=[64])
    parser.add_argument('--simrank_pre_cal_file', type=str, default=None,
                        help='Pre calculation file for simrank')
    parser.add_argument('--propa_mode', type=str, default='post',
                        help='Propagation mode')
    parser.add_argument('--skip_factor', type=int, default=1,
                        help='skip factor in propagation')
    parser.add_argument('--simrank_file_name', type=str, default=None,
                        help='Pre calculation file for simrank')
    parser.add_argument('--simrank_guided', type=int, default=1,
                        help='If use simrank for aggregation')